...
tags:
  ...
  - Heroku
  - imgur
headerImage: /images/blog/R0GFe6X.png
...
![HerokuダッシュボードのAddonsからHeroku PostgreSQLを選択する画面](/images/blog/hkpshVq.png)

こちらレプリケーションやレコード上限がありますが、無料枠があります。**10000レコードを超えると課金対象**になるので、ならないように定期レコード削除機能も開発する必要がありますね。

## アーキテクチャ

![arch](/images/blog/iz9IpHf.png)
...
![Heroku PostgreSQLの接続情報を確認するCredentials画面](/images/blog/TR9WSti.png)

こちらの情報を控え、Hasura CloudのコンソールからDataを選択し、新しいDababaseの情報を入れ込みます。執筆しながら気が付いたのですがよく見たらHeroku PostgreSQLが作れるメニューがありますね..。

![Hasura CloudのData画面でDB接続設定を行う画面](/images/blog/vzpBanB.png)

無事接続が完了すると、このようにスキーマやテーブルが作れるようになります。

![Hasura Cloudでスキーマとテーブルの作成が可能になった画面](/images/blog/c1BI53V.png)

テーブルを作ってみましょう。create tableから簡単に作れます。

![Hasura Cloudのcreate tableでテーブルを作成する画面](/images/blog/dm0FJPU.png)

default valueにnow()などの関数が入れられますのでtimestampはこちらで作っちゃうことにしました。便利ですね。

無事接続ができて適当な行を作ればこのようにメインコンソールからGraphQLが実行できるようになります。

![Hasura CloudのGraphQLコンソールでQueryを実行した結果](/images/blog/R0GFe6X.png)
...
![Hasura Cloudのテーブル設定画面でanonymousロールのpermission設定](/images/blog/gvA0pe2.png)
...
![Hasura Cloudの環境変数にUNAUTHORIZED_ROLEを設定する画面](/images/blog/rRAVwPw.png)
...
![Hasura CloudのSecurityからAPIリクエストのリミットを設定する画面](/images/blog/FvSFqnx.png)
...
![Next.jsで構築した植物モニタリングダッシュボードの画面](/images/blog/YznBG45.png)
...
![Cypress Scenario Recorder拡張で要素セレクタを取得する画面](/images/blog/KaTx1pD.png)
...
![Slackに毎日自動投稿された植物ダッシュボードのスクリーンショット](/images/blog/xNW7WDG.png)
