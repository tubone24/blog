...
description: "Hasura CloudとHeroku PostgreSQLを連携し、無料でGraphQLエンドポイントを構築する方法を解説。Pythonでのmutation実装、Next.js+Apollo Clientでのダッシュボード開発、Cypressでの画面キャプチャによるSlack通知まで一気通貫で紹介します。"
tags:
  - Hasura
  - GraphQL
  - Vercel
...
![Hasura CloudのWebコンソールでGraphQLを操作するデモ](https://raw.githubusercontent.com/hasura/graphql-engine/master/assets/demo.gif)
...
![HerokuダッシュボードのAddonsからHeroku PostgreSQLを選択する画面](https://i.imgur.com/hkpshVql.png)
...
今回はこのようなアーキテクチャにします。以前[Raspberry PIを使って植物の水やり監視システムを作る](/2020/05/10/plant-check/)で作ったラズパイ水やり管理システムのメトリックを定期的に収集してHasuraのGraphQL mutationを介してHeroku PostgreSQLに書き込み、それをVercel + Next.jsで作ったダッシュボードでこれまたHasura GraphQLを介してQuery取得し可視化する、というものです。
...
![Heroku PostgreSQLの接続情報を確認するCredentials画面](https://i.imgur.com/TR9WSti.png)

こちらの情報を控え、Hasura CloudのコンソールからDataを選択し、新しいDababaseの情報を入れ込みます。執筆しながら気が付いたのですがよく見たらHeroku PostgreSQLが作れるメニューがありますね..。

![Hasura CloudのData画面でDB接続設定を行う画面](https://i.imgur.com/vzpBanB.png)

無事接続が完了すると、このようにスキーマやテーブルが作れるようになります。

![Hasura Cloudでスキーマとテーブルの作成が可能になった画面](https://i.imgur.com/c1BI53V.png)

テーブルを作ってみましょう。create tableから簡単に作れます。

![Hasura Cloudのcreate tableでテーブルを作成する画面](https://i.imgur.com/dm0FJPU.png)

default valueにnow()などの関数が入れられますのでtimestampはこちらで作っちゃうことにしました。便利ですね。

無事接続ができて適当な行を作ればこのようにメインコンソールからGraphQLが実行できるようになります。

![Hasura CloudのGraphQLコンソールでQueryを実行した結果](https://i.imgur.com/R0GFe6X.png)
...
![Hasura Cloudのテーブル設定画面でanonymousロールのpermission設定](https://i.imgur.com/gvA0pe2.png)
...
![Hasura Cloudの環境変数にUNAUTHORIZED_ROLEを設定する画面](https://i.imgur.com/rRAVwPw.png)
...
![Hasura CloudのSecurityからAPIリクエストのリミットを設定する画面](https://i.imgur.com/FvSFqnx.png)
...
他はmeterial table使ったり、chart.js使ったりしてますがこちらは以前作った[Next.jsとVercelとRecoilとMaterial Tableを使ってAWSのステータスダッシュボードを作ってみた話](/2021/01/11/vercel-next/)のパクリコードなので解説は割愛します。
...
![Next.jsで構築した植物モニタリングダッシュボードの画面](https://i.imgur.com/YznBG45.png)
...
![Cypress Scenario Recorder拡張で要素セレクタを取得する画面](https://i.imgur.com/KaTx1pD.png)
...
![Slackに毎日自動投稿された植物ダッシュボードのスクリーンショット](https://i.imgur.com/xNW7WDG.png)


## 反省

Denoを使ってSlackアップロードスクリプトは改修します。絶対に。（実際に[Denoを使ってGraphQLを叩きながらSlackに投稿する](/2021/07/28/deno-slack/)記事で改修しました。）
