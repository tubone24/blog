---
slug: 2024/12/30/building-langfuse-v3-with-aws-managed-services
title: "Langfuse v3はv2からどのように変わったのかを噛み締めながらAWSマネージドサービスでLangfuse v3を作りきる"
date: 2024-12-31T02:35:41+0000
description: "Langfuse v3のClickHouse導入・Redis非同期キューイング・ReplacingMergeTreeの仕組みをDeep Diveし、App Runner・Aurora Serverless・ECS Fargate・EFS・ElastiCacheを組み合わせたAWS構成をTerraformで構築する全過程を解説します"
tags:
  - AWS
  - Langfuse
  - ClickHouse
headerImage: /images/blog/umLZxXJ.jpg
...
![Langfuse v2 Architecture AWS](/images/blog/RWzEbMz.jpg)
...
ありがたいことにLangfuse v2はNext.jsで構築されていることから**リクエストベースでコンテナが実行されればよいため**、（常駐する必要がないため）コスト最適化の観点でも**AWS App Runner**でデプロイしておりました。ECS FargateでのElasticsearch運用については[企業名サジェスト機能の貧乏開発](/2024/12/05/cost-effective-company-name-suggestion-feature/)でも取り上げており、小さなコンテナを使い倒すノウハウが活きています。
...
![Langfuse v2 Architecture](/images/blog/3pYyKr7.png)

そしてv3がこちらです。

![Langfuse v3 Architecture](/images/blog/IIFnpvS.png)
...
![昨今のLLMアプリケーションのObservation特性](/images/blog/gJGFk0v.png)
...
![プロンプトマネジメントAPIの低レイテンシーが必要な理由](/images/blog/9G3pUC7.png)
...
![Redisをキューとして挟むことでトレースの書き込みを非同期化](/images/blog/jkLzM8V.png)
...
![レコードの一部カラムを断続的に更新する場合](/images/blog/nhx9rwz.png)
...
![Workerでのマージ処理によるレコードの一貫性の保持](/images/blog/m5qsPe9.png)
...
![Langfuse v3 Architecture AWS](/images/blog/umLZxXJ.jpg)
...
Before
![Langfuse v2.92.0](/images/blog/IKTkzIK.png)

After
![Langfuse v2.93.7](/images/blog/o2rx7t9.png)
...
![Langfuse v2 Architecture AWS](/images/blog/kVV2hGJ.png)

こちらに新しいコンポーネントを足し込んでいきます。

![Langfuse v3 Architecture AWS](/images/blog/CdXzO96.png)
...
![ClickHouseへのアクセス経路](/images/blog/nrmKKZj.png)
...
![Langfuse ServerをECS化](/images/blog/VYbMEY3.png)
...
![ClickHouseへのアクセス経路](/images/blog/kHp2OOt.png)
...
![ClickHouseのデータ永続化](/images/blog/yekxuN3.png)
...
![EFSのマウント失敗](/images/blog/AuhZn4M.png)
...
![Langfuse画面上部にBackground Migrationsのステータスを示す緑色バッジが表示されている](/images/blog/j7YZLwB.png)
...
![Background Migrationsの進捗状況画面で各マイグレーションのステータスが一覧表示されている](/images/blog/NlPszng.png)
...
![PostgreSQLのbackground_migrationsテーブルでstatusがSuccess/Activeのレコード一覧](/images/blog/9RD4od5.png)
...
![SQLでstatusをFailedに変更した後のbackground_migrationsテーブル](/images/blog/sHiLXMC.png)

すると、**retryボタンが押せる**ようになり、再度マイグレーションを実行できます。

![Background MigrationsのUI上でRetryボタンが押せるようになった画面](/images/blog/TG6QTAq.png)
...
2025年はもっとLLMを活用したアプリケーション開発が促進されることでしょう。今のうちにLangfuse v3のアーキテクチャを理解・移行しておくことで、よりスムーズなLLMアプリ開発ができることでしょう。なお、構築後のコスト最適化については[Langfuse v3を安く運用する方法](/2025/02/16/cost-effective-langfuse-v3-deployment/)で詳しくまとめています。
