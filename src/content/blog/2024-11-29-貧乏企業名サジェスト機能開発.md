---
slug: 2024/12/05/cost-effective-company-name-suggestion-feature
title: "企業名サジェスト機能貧乏開発"
date: 2024-12-05T14:24:04+0000
description: "ECS FargateとElasticsearchのN-gram/Edge N-gramトークナイザーを活用し、小さなコンテナでも高性能な企業名サジェスト機能をコスト効率よく構築する方法を、インデックス設計からデバウンス処理まで詳しく解説します"
tags:
  - Elasticsearch
...
![AWS re:inventが開催されているラスベガスの風景](https://i.imgur.com/wuH4vwW.jpg)
...
![企業名を入力するとサジェスト候補がリアルタイムに表示されるUIのデモ](https://i.imgur.com/1UvKYKu.gif)
...
今回は、 AWSなどのクラウドサービスにスクラッチでサジェスト機能の全文検索の仕組みとフロントエンドを作成し、しかもそのコストをできるだけ下げていく、というなんともマニアックな要件に対して挑んだ軌跡を記載していきたいと思います。[Langfuse v3のAWS構築](/2024/12/30/building-langfuse-v3-with-aws-managed-services/)でもECS Fargateを活用した構成を採用しており、小さなコンテナの使い方に通じる部分があります。
...
![Elasticsearchを使ったサジェスト機能のアーキテクチャ概要図](https://i.imgur.com/OO7JYK3.jpg)
...
![Material UIのAutocompleteコンポーネントによるサジェスト動作のデモ](https://i.imgur.com/1WX3Roe.gif)
...
![デバウンス300msでの企業名サジェスト動作デモ](https://i.imgur.com/TFdYdcS.gif)

**50ms**

![デバウンス50msでの企業名サジェスト動作デモ、より機敏に反応している](https://i.imgur.com/fQe66X3.gif)
...
もちろん実運用だと可用性を考え冗長な構成を取ったりする必要はあるので**あくまでもケースバイケース**ということはご承知いただくのと、お金があるのであれば運用コストを考えてOpenSearchなどのマネージドサービスに乗っかったほうが100％よいとは思います。コスト最適化の考え方は[Langfuse v3を安く運用する方法](/2025/02/16/cost-effective-langfuse-v3-deployment/)でも深掘りしています。
...
![N-gramとEdge N-gramによる転置インデックスの分割イメージ図](https://i.imgur.com/qH1P0nD.jpg)
...
![ふりがなと企業名のマルチマッチング検索による重み付けの概念図](https://i.imgur.com/1HLTLQL.jpg)
...
![ECS Fargateに小さなElasticsearchコンテナを配置したインフラ構成図](https://i.imgur.com/ioPuQHy.jpg)
...
![Elasticsearchのアナライザー処理フロー：Character Filter → Tokenizer → Token Filterの順で処理される図](https://i.imgur.com/Nuvz5Ik.png)
