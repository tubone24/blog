...
description: "MQTTプロトコルとRaspberry Pi接続の電子ペーパーを使い、Webから送信した年賀状を電子ペーパーに表示するシステムの構築方法を解説。FastAPIによるバックエンド、React Hooks＋Tailwind CSSのフロント実装、mosquittoブローカーの構成を紹介します。"
tags:
  - MQTT
  - RaspberryPi
  - 年賀状
...
![斎藤佑樹の「考えてもしょうがないBOX」記事のスクリーンショット](https://i.imgur.com/BU7o9ke.jpg)
...
今回電子ペーパーを実装した端末がRaspberry Piということもあり、コア技術はMQTTで行くことにしました。Raspberry Piの活用は[植物の水やり監視システム](/2020/05/10/plant-check/)でも取り組んでおり、IoTの幅を広げていきたいところです。

### React Hooks

[React Hooks](https://reactjs.org/docs/hooks-intro.html)も今更？感ありますが、Reactのお勉強は[Gatsby.jsでのブログ実装](/2019/09/01/netlify-and-gatsby/)以来やっていないので、ちょっと動向に触れることにしました。
...
![MQTT年賀状システムのアーキテクチャ図：フロント→FastAPI→MQTTブローカー→電子ペーパー](https://i.imgur.com/E3hyjnA.png)
...
![React博士のイラスト（質問を受けている）](https://i.imgur.com/zrdOCVi.png)

> 知らね。自分で考えろ

![React博士のイラスト（突き放す表情）](https://i.imgur.com/HnLNjda.png)
...
というかEKSに載っけるなら最初からAWS IoTにすればよかったですね。電子ペーパーのRaspberry Pi活用は、後日[電子ペーパーカレンダーの製作](/2021/07/26/calendar/)でも発展させています。
...
![年賀状作成フォーム：名前・タイトル・メッセージ・画像の入力欄](https://i.imgur.com/k4Ti9nI.png)

**名前**、**タイトル**、**メッセージ**と**画像**を入力フォームに入れていきます。Previewボタンを押すと相手に送信する年賀状の**プレビュー**ができます。

![年賀状のプレビュー画面：送信前に完成イメージを確認できる](https://i.imgur.com/lEjbh7L.png)

問題なければ、**Submit**します。

相手がTopicをSubscribeしていれば、このように電子ペーパーに年賀状が描画されます。

![電子ペーパーにMQTT経由で年賀状が描画されるデモ動画](https://i.imgur.com/LXEcbLD.gif)

## 改めまして

![完成した電子ペーパー年賀状の表示例](https://i.imgur.com/WJnsQlT.png)
