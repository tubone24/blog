...
この記事では、まず**実際にmspecで小さな機能を1つ作ってみる流れ**を見てもらい、そのあとで設計思想と、背景にある [Kent BeckのTDD](https://www.oreilly.com/library/view/test-driven-development/0321146530/) や [Daniele ProcidaのDiátaxis](https://diataxis.fr/)、 [GitHub Spec Kit](https://github.com/github/spec-kit) と [OpenSpec](https://github.com/Fission-AI/OpenSpec) の系譜まで踏み込んでいきます。
...
同時に**mspec Web UIが`localhost:3847` でバックグラウンド起動**して、これ以降に生成される成果物やテスト結果をブラウザで眺められます。

### 止まるたびにアーティファクトを確認する

mspecのワークフローは、Claude Codeの**スキル**として定義されています。

以降打つコマンドは`/mspec:continue`のひと言ですが、開発者の仕事は、**ステップが止まったときにアーティファクトを確認し、意図と合っているか判断すること**に集中します。
...
現在のデフォルトは次の**12ステップ** で、黄色が `block: true`、つまり **人間確認ポイント** です(これはプロジェクトごとに変更できます)。
...
proposalフェーズでは、先ほど作られた概要を元にClaude Codeから`AskUserQuestion`の形で、「どんな検索機能が欲しいか」「検索対象のドキュメントは何か」といった質問が次々飛んできます。

あなたは「`docs/` 配下のMarkdownを全文検索できるようにしたい」「結果にはスニペットを出したい」といくつか提示される選択肢に返答するだけで、`proposal.md`（`Explanation`）が生成されます。

中を開くと、Why・Goals・Non-Goals・Open Questionsがこんな形で整理されています。
...
>
> - **Open Questions:** インデックスの更新タイミング（ビルド時？起動時？）→ research フェーズで調査

意図と食い違っていなければ `/mspec:continue`。違っていればAIに再度返信する形です。

膨大なMarkdownを自分で書く必要はほとんどありません。

#### 仕様として形になっているか確かめる

次のフェーズは、delta specを作成する段階です。今回の要求（proposal）を満たす**変更の**仕様を書いていきます。

FRは **EARS（Easy Approach to Requirements Syntax）** という記法で書かれます。「〜できる」という曖昧な文体ではなく、**条件・主語・振る舞いが一意に決まる形式**です。

> - `FR-001`: `WHEN` ユーザーがキーワードを入力したとき、システムは `docs/` 配下のMarkdownを全文検索し、マッチしたドキュメントの一覧を返す `SHALL`
> - `FR-002`: `WHEN` 検索結果が表示されるとき、システムはマッチ箇所のスニペットをハイライト付きで表示する `SHALL`

と要件が整理された `spec.md`（`Reference`）が出てきます。
...
この2層の記法が重要なのは、**この仕様が変更全体のテスト設計の唯一の起点になる**からです。

後続の実装タスクも、E2Eテストも、ここに書かれたシナリオから派生します。 逆に言えば、ここで仕様が曖昧なまま進むと後からの手戻りが大きくなるということです。

EARSとGherkinで書かせることで、その曖昧さを仕様フェーズで潰す——それがmspecの考え方です。

全部通読するより、**インデックス構築のFRが抜けていないか**、**受け入れシナリオは実際に確認できる形か**を引きながら確認すると良いでしょう。

#### 設計とアーキテクチャを確かめる

次のフェーズは調査と設計です。

`research.md` で「既存の `docs/` ローダーを使い回せるか」「FlexSearchとMinisearchどちらが軽量か」といった調査が行なわれます。

調査結果は **論点・採用案・代替案・根拠** の4列テーブルで整理されており、「なぜその案を選んだか」の根拠が変更後も参照できる形で残ります。

proposalで決めきれていないOpen Questionもここで調査され、**決定事項として整理される**ので、後から見返したときに「なぜこの設計になっているのか」を追いやすくなります。

**ドキュメント全文検索の機能を足す** 例だと調査結果はこんな感じです。
...
ただし、このドキュメントは`Refernce`なので気になるところがあれば確認する、程度で大丈夫です。

この調査を元に`design.md`（`Reference`）が生成されます。

**検索APIは既存の `/api/docs` ルートに乗せる**といった技術方針とコンポーネント間の依存関係が整理されています。

アーキテクチャの大きな判断はここで確認しておくと、後半の実装フェーズで手戻りが減ります。

`design.md` と同時に `design-rationale.md` と `architecture-overview.md` も生成されます。

- `design-rationale.md`（`Explanation`）は、**なぜその設計を選んだのかの意思決定の背景**を語る文書です。全文検索の例なら「なぜ `src/search/` に独立モジュールを切ったのか」「なぜFlexSearchを採用したのか」といった判断の経緯が読めます。
- `architecture-overview.md` は、コンポーネント間の依存関係を**俯瞰した全体図**のドキュメントです。実装前に「全体像はこうなる」というイメージを持てます。
- `quickstart.md`（`How-to`）は、実装完了後に**ゴールデンパスを手で確認するための手順書**です。「`localhost:3000/search` を開いてキーワードを入れると結果が出る」という確認の流れが書かれているので、実装が終わった後に実際に動かして確認できます。

これらのドキュメントは同じ設計情報を**異なる切り口**で表現しています。

細部にまでフォーカスしたければ、`design.md`を見て、設計の意図を理解したければ`design-rationale.md`を読むという選び方ができます。

#### 実装前にチェックリストを押さえておく

`checklist.md` にはFRのカバレッジ、リグレッションリスク、動作確認項目が整理されています。

このチェックリストは、基本的に人間が確認するためだけのものではありません。
...
最後の `archive` フェーズでDelta Specが正式な仕様（SoT）にマージされて、1つの変更が完了します。
...
mspecは3つのMを軸に設計されています。その設計意図も含めてご紹介します。
...
ここで一度、 [Spec Kit](https://github.com/github/spec-kit) と [OpenSpec](https://github.com/Fission-AI/OpenSpec) の流れに位置付けて整理します。

## オマケ機能

3つのMが骨格ですが、ドッグフーディングを続けるなかで「これも欲しい」と足してきた機能がいくつかあります。
...
そうして曖昧なまま作られたUIは、最後の実装フェーズで実物を触ってはじめて「あれ、思ってたのと違う」となりがちです。それならバイブコーディングで実装と確認のループをたくさん回したほうが圧倒的に早いし楽です。
...
**どの要件がどれだけ危ないか、影響がどこまで及ぶか**に色をつけておくわけです。Tierごとにproposalフェーズで質問される内容、特にセキュリティ周りの質問の数や質が変わってきます。

ですが、mspecを使ってもセキュリティ的な観点の洗い出しはまだ甘いですし、リスクの線引きを固定ルールで縛るべきか、AIに柔軟に判断させるべきかは、自分のなかでもまだ答えが出ていません。

### アーカイブから学ぶ

仕様駆動で進めていて、**実装フェーズあたりで違ったと気づく**ことってありませんか。
