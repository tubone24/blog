...
description: "Gatsby.js+TypeScript+Netlifyで構築した個人技術ブログの技術スタックを全解説します。Preact導入によるバンドルサイズ削減、Bootstrap+PurgeCSSの最適化、Algoliaによるブログ内検索など無料で実現する構成を紹介します。"
tags:
  - Gatsby.js
  - TypeScript
  - Bootstrap
  - Sass
  - Netlify
headerImage: /images/blog/18P6Wlz.png
...
![2023年の干支であるうさぎのイラスト](/images/blog/cRHAJbo.png)
...
また、対象の読者は主に個人開発で**無料でできるだけそれっぽいサイトが作りたい**という人です。品質面では[textlintを導入してCI上で日本語校正](/2022/4/22/textlint/)も行なっています。無料にこだわってます。めっちゃこだわってます。高度なことはお金を払ってしましょう...。
...
![チェンソーマンの未来の悪魔が「最高」と叫ぶシーン](/images/blog/Cv8jtro.jpg)
...
<SEO title={frontmatter?.title} 
     ...
     description={excerpt || ""}
     image={frontmatter?.headerImage || "/images/blog/4r1DViT.png"}
/>
```
...
![tw-del-backupで復元したPreact導入時のバンドルサイズ削減報告ツイート](/images/blog/Pb4nL6Q.png)
...
![webpack-bundle-analyzerによるProduction環境のバンドルサイズ可視化結果](/images/blog/ssbwpVo.png)
...
![Algoliaを使ったブログ内検索のSearchboxコンポーネントの動作デモ](/images/blog/uebtAld.gif)
...
![Lighthouseでアクセシビリティ100点を達成したスコア画面](/images/blog/gXIfewW.png)

ちなみにa11yってなんて読むんですか？「アリー？」「エーイレブンワイ？」

### Sentry

一応監視系もしっかりしたほうがええな、ということで[Sentry](https://sentry.io)を導入してます。Sentryの基本的な使い方は[Sentryを使ってフロントエンドのエラーを確認する](/2019/09/22/sentry/)に、新機能のSession Replayについては[Sentry新機能のReplayを使ってみる](/2023/01/18/sentry-replay/)にまとめてあります。

ほとんどが**ChunkLoadError**ですが、たまにライブラリーアップデート起因のエラーもでるのですごく重宝してます。

![Sentryダッシュボードに表示されたブログのエラー一覧](/images/blog/IOVq3l.png)

あと、**パフォーマンスメトリック**も取っているのでパフォーマンス改善への意欲づくりに貢献してます！意欲だけあって実行しないですが。

![Sentryのパフォーマンスメトリック画面でWebVitalsのスコアを表示](/images/blog/GSaWx2D.png)
...
![GTmetrixでグレードAを取得したブログのパフォーマンス計測結果](/images/blog/whYjszk.png)

Performance metricもおおむね良さそうです。

![GTmetrixのPerformance Metricsで各指標がグリーン判定を示した結果](/images/blog/y7N1fDH.png)

ただ、Lighthouse([Page Insight](https://pagespeed.web.dev))で測ってみるとデスクトップはまずまずですが、**モバイルの結果がすこぶる悪い**です。

![Lighthouseのデスクトップ版スコアで高得点を示した結果](/images/blog/zQ0pt29.png)

![Lighthouseのモバイル版スコアでパフォーマンスが低く出た結果](/images/blog/TEMPeDd.png)
...
![Google Analytics 4のSearch Console Insightで表示された流入検索ワードランキング](/images/blog/32aZAir.png)
