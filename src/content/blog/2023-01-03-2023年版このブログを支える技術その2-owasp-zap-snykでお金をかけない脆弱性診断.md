...
description: "Snyk CLIによる依存関係・静的コード解析とOWASP ZAPによるアクティブスキャンをGitHub Actionsに組み込み、個人開発でも無料で継続的な脆弱性診断を実現する方法をハンズオン形式で解説します"
tags:
- Snyk
- OWASP ZAP
- 脆弱性診断
...
今回はセキュリティ編、ということで[Snyk](https://snyk.io/)と[OWASP ZAP](https://www.zaproxy.org/)を使って **お金をかけない脆弱性診断** を個人開発でも実現できる方法で検討していきたいと思います。コスト削減の工夫は[企業名サジェスト機能の貧乏開発](/2024/12/05/cost-effective-company-name-suggestion-feature/)でも取り上げていますが、セキュリティも同じくお金をかけずに実現できます。
...
![Snykの脆弱性レポート画面で修正方法やリファレンスが表示されている様子](https://i.imgur.com/g5fWDjA.png)

Depandabotも書いてある情報は大差ないのですが、なんとなくSnykのほうが見やすい..。

![Dependabotの脆弱性アラート画面との比較](https://i.imgur.com/oGOC92Y.png)
...
![SnykダッシュボードでGitHubリポジトリ連携後に脆弱性が自動検出されている画面](https://i.imgur.com/he9u9kb.png)
...
![ブログ記事下部に表示されるTwitter・FacebookのShareBoxコンポーネント](https://i.imgur.com/w86sKvc.png)
...
力技ですが上のようにSnykコマンドの標準出力をいったんtxtで吐き出して、必要なフォーマット・エスケープ処理を行なったのち、結果をon.pull_requestイベントの際に取れる、github.event.pull_request.comments_urlにPOSTします。GitHub Actionsを使ったCI/CDの構成は[textlintの導入](/2022/4/22/textlint/)でも活用しており、PRに自動でフィードバックを返す仕組みは開発体験の向上にとても効果的です。

![GitHub PRにSnykの脆弱性レポートがコメントとして自動投稿されるデモ](https://i.imgur.com/mUDXQfl.gif)
...
![OWASP ZAP Web GUIのスプラッシュ画面](https://i.imgur.com/7rIrR1o.png)
...
![OWASP ZAPのAutomated Scanでターゲット設定とStandardモードを選択する画面](https://i.imgur.com/rs9HIta.png)
...
![OWASP ZAPでAutomated ScanのSpider設定画面](https://i.imgur.com/Moc4POV.png)
...
![Ajax Spiderがサブツリーのエンドポイントを収集している進行画面](https://i.imgur.com/i0OIcxd.png)

Spiderが終わるとそのまま**Passive Scan、Active Scan**に入ります。検知されたURLの量にもよりますが30分から1時間くらいは平気でかかります。

![OWASP ZAPのPassive ScanとActive Scanが進行中のスキャン画面](https://i.imgur.com/kNtlibG.png)
...
![SQL InjectionやXSSなどの攻撃パターンが実行されているログ画面](https://i.imgur.com/wNJxvr0.png)
...
![技術ブログのコンテンツが原因で誤検知が発生したスキャン結果画面](https://i.imgur.com/YguJMkA.png)
...
![GitHub Pagesにデプロイされた OWASP ZAPの診断レポートHTML](https://i.imgur.com/FnGbBym.png)
...
![「今日も一日ご安全に」のイメージ画像](https://i.imgur.com/1vEoOf5.jpg)
