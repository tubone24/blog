...
description: "JSON ResumeとAPI With GitHubを使って職務経歴書をJSON APIとして公開する方法を解説。JSON Resumeのスキーマ各項目（職歴・学歴・スキルなど）の書き方を海外事例も交えて紹介し、resume-cliでHTMLレジュメを生成する手順もまとめました。"
tags:
  - JSON Resume
  - GitHub Pages
  - Resume
...
![API With GitHubのレポジトリ作成画面](https://i.imgur.com/6Lti5I8.png)

あとは普通にレポジトリを新規作成する手順で作っていきます。

![API With GitHubでレポジトリを新規作成する手順画面](https://i.imgur.com/d4nmRXZ.png)

Make APIボタンを押すと・・・。

![API With GitHubのMake APIボタンを押した画面](https://i.imgur.com/W3Paou3.png)

新規JSONファイルを作る画面に進みます。

![API With GitHubの新規JSONファイル作成画面](https://i.imgur.com/vzhzqiI.png)

JSON ResumeのJSONは `resume.json`がデフォルトなので `resume.json`で作ります。

![API With GitHubでresume.jsonファイルを作成した画面](https://i.imgur.com/ay4H67M.png)
...
![API With GitHubでJSON Resume入力が完了した編集画面](https://i.imgur.com/PSokx4v.png)

LinkからAPIにアクセスしてみましょう!!!

![JSON ResumeのAPIレスポンスがブラウザに表示されている画面](https://i.imgur.com/wF82uti.png)
...
![resume-cliで生成したHTMLテンプレートのレジュメ表示画面](https://i.imgur.com/IIzgYrK.png)

かっこいい!!!感じでResumeがサクサク作れてしまいます。

## 結論

よく考えたら、海外向けのResumeを作るほど海外志向ないのでただの海外就活サイトの読み漁りになりました。GitHub Pagesを活用した自己紹介系のプロジェクトとしては、[自己紹介をnpxでやってみた話](/2022/03/05/npx/)や、[GitHub ProfileにREADMEをつける方法](/2020/07/12/github-profile/)もあわせてどうぞ。
