...
description: "2020年5月のDatadog Agent障害を題材に、SSL証明書の仕組みからクロスルート証明書までを詳しく解説します。公開鍵暗号・認証局・証明書チェーン・失効リストの基礎知識とともに、AddTrust External CA Root失効の原因と対処法を紹介します。"
tags:
  - Datadog
  - SSL
...
![Slackに大量のDatadog監視アラートが通知されている画面](https://i.imgur.com/MQtTdnk.png)
...
![暗号技術入門 第3版 秘密の国のアリスの書籍表紙](https://i.imgur.com/E0GC9w0.jpg)

さて簡単にSSL通信のおさらいをします。

![SSL/TLS通信の仕組みを示すイラスト（クライアントとサーバー間の鍵交換プロセス）](https://i.imgur.com/APQPDxd.png)
...
![AWSコンソールのSSLサーバー証明書の詳細情報画面（X.509形式）](https://i.imgur.com/r3Rhva4.png)
...
![SSL証明書の有効期限切れ警告が表示されたブラウザ画面](https://i.imgur.com/m4xqMek.png)

証明書の有効期限はOpenSSLコマンドで簡単に取れますので、そういうことがないように[Elasticsearchの監視](/2017/7/23/elasticsearch-zabbix/)と同様にZabbixのExternalScriptとかできっちり管理することをおすすめしますー。
...
![ZabbixでSSL証明書の有効期限を監視しているグラフ画面](https://i.imgur.com/KzVk6D2.png)
...
![認証局と代紋の比喩イラスト（上位組織が下位を認証する関係）](https://i.imgur.com/RfBcxD9.png)
...
![認証局による証明書発行の流れを代紋の比喩で説明した図](https://i.imgur.com/30262FQ.png)
...
![証明書失効リスト（CRL）を破門状に例えたイメージ写真](https://i.imgur.com/f382RVO.jpg)
...
![tubone-project24.xyzのSSL証明書情報（Let's Encrypt Authority X3発行）](https://i.imgur.com/oh5oyKC.png)
...
![ブラウザで表示した証明書チェーンの階層構造（赤枠で囲んだ部分）](https://i.imgur.com/wDLRAKk.png)

赤枠がチェーンです。

**Let's Encrypt Authority X3**の証明書を見てみましょう。

![Let's Encrypt Authority X3の証明書詳細（発行者がDST Root CA X3）](https://i.imgur.com/gpaXTQe.png)
...
![DST Root CA X3のルート証明書詳細（自己署名証明書）](https://i.imgur.com/m8zYyuz.png)
...
![Macのキーチェーンアクセスでインストール済みルート証明書の一覧を表示した画面](https://i.imgur.com/VGk9uYk.png)
...
![通常の証明書チェーンの図（サーバー証明書→中間証明書A→ルート証明書A）](https://i.imgur.com/JcUkWaJ.png)
...
![ルート証明書が更新された場合のチェーン図（新しいルート証明書Bに切り替え）](https://i.imgur.com/1uF5dA3.png)

と新しい**ルート証明書B**を使うことになりますね。

ではさらにこの端末がアップデートできない場合やインストールしてないルート証明書へアクセスする場合はどうなるでしょうか？

![端末のルート証明書が古い場合に検証失敗する様子を示した図](https://i.imgur.com/y0ul2Eg.png)
...
![クロスルート証明書を使って古いルート証明書Aから検証を完了する仕組みの図](https://i.imgur.com/9fwz8jM.png)
...
![Javaのロゴと「30億のデバイスで走るJava」のキャッチコピー](https://i.imgur.com/zKzF2Wa.jpg)
...
Datadogへメトリックを送信しなければいけないので当然インターネット越しに通信をしなければいけません。[AWS X-RayとDatadog APMの連携](/2020/1/20/x-ray-datadog/)でも触れましたが、もちろん通信はSSLで暗号化されてしかるべきです。
...
![Datadog Agentが起動しTornadoサーバーがlocalhost:17123でリッスンしている画面](https://i.imgur.com/Z58AGfp.jpg)
...
![Datadog Agentのアーキテクチャ図（Collector・Forwarder・DogStatsDの構成）](https://i.imgur.com/q1Ouk6Y.png)
...
![Sectigoの証明書チェーン構造図（USERTrust RSAとAddTrust External CA Rootの関係）](https://i.imgur.com/oMJmF1b.jpg)
...
![OpenJDK9のJEP319に含まれるAddTrust External CA Rootルート証明書の一覧](https://i.imgur.com/yn4VUB3.png)
