...
ちゃんと大学でRDBの正規化とか習った人からすると、「正規化されてないやんけ...」と引いてしまうやつなんですが、クエリパターンに応じた非正規化が**正解**だったりします。
...
仮にtracesが1億行あれば1億エントリのハッシュテーブルをメモリに展開しないといけないし、そのうえ `join_algorithm = 'auto'` を明示設定している場合は、メモリが足りなくなると**より遅いディスクベースのアルゴリズムのマージ結合になる場合もあります**。
