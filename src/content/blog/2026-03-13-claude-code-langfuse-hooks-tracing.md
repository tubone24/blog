...
tags:
  ...
  - Langfuse
  - LLMOps
headerImage: /images/blog/YESLGDQ.png
...
![Langfuse上でツール実行エラーがERRORレベルで表示されている画面](/images/blog/sbFCc5n.png)
...
![LangfuseのGeneration inputにExtended Thinkingのテキストが記録されている画面](/images/blog/BAmXun6.png)

ところが、ある日を境にClaude Codeの仕様変更で、transcriptのthinkingフィールドが**空文字列**になりました。

thinkingブロック自体は残っているんですが、**中身のテキストが記録されなくなった**のです。

![仕様変更後にthinkingフィールドが空文字列になったtranscriptの例](/images/blog/E6PElfJ.png)
...
![Langfuseのトレース全体像（桜の開花日予測プロンプトに対するツール呼び出しの一覧）](/images/blog/Zoa2C1W.png)

まず、最初のGeneration（`llm:streaming`）で「横浜の桜の開花日を予測するために、複数のエージェントで並行して徹底調査するわん！」というテキストが出力されます。

![Claude CodeのターミナルでLLMのストリーミング出力が表示されている画面](/images/blog/s8yS9h4.png)

ちょうどClaude Codeでも同じテキストが表示されているのがわかりますね。

![LangfuseのGeneration outputにLLMの応答テキストが記録されている画面](/images/blog/Hep0NcR.png)
...
![Langfuseでサブエージェントが親Spanとして表示され子ツールがぶら下がる構造](/images/blog/YESLGDQ.png)
...
![WebSearchツールの入力クエリと検索結果がLangfuseに記録されている画面](/images/blog/psfCyNt.png)

### サブエージェントのアウトプット

サブエージェントは、ツールから得られた情報をもとに分析を行ない、最後に `last_assistant_message` として調査レポートを出力します。Langfuse上ではサブエージェントのアウトプットとして表示されるわけですね。

![サブエージェントの調査レポートがLangfuseのSpanアウトプットに記録されている画面](/images/blog/FTYDz8k.png)

また並列で動いていた別のサブエージェントでは、2026年2月下旬〜3月中旬の気象状況をまとめていることもわかります。

![並列で動く別のサブエージェントが気象状況をまとめた結果のLangfuse画面](/images/blog/SYIoXwk.png)
...
![llm:end_turnのGenerationにサブエージェントの調査結果を統合した最終回答が記録されている画面](/images/blog/9nEH2qO.png)
...
![PermissionRequestイベントがLangfuseにEventとして記録されている画面](/images/blog/Cs5RQqv.png)
