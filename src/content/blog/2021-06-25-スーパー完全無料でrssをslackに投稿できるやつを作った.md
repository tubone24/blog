...
description: "HarperDB・feedparser・GitHub Actionsを組み合わせて、完全無料でRSS更新をSlackに自動通知するシステムを構築。OGP画像取得やキーワード自動抽出機能も実装し、技術ブログのキャッチアップを効率化する方法を解説します。"
tags:
  - RSS
  - HarperDB
  - feedparser
  - termextract
headerImage: /images/blog/Ip4IaYs.png
...
日本ではあまり聞きませんが、[dev.to](https://dev.to/)とかだとちょこちょこ話題に上がっております。以前[GoでAWS LambdaからGraphQLを叩いた記事](/2019/11/26/go-lambda/)でもGraphQLを使いましたが、今回はDBとしてHarperDBを選択しています。

こちらのHarperDB、HarperDB Cloud Instanceというマネージドサービスも提供されており、インスタンスタイプを選ぶだけで、手軽にHarperDBを使うことができるようになっております。

![HarperDB Cloud Instanceのプラン選択画面](/images/blog/CA1sLCU.png)

![HarperDB Cloud Instanceの無料プラン詳細](/images/blog/48qXVQw.png)
...
![TwitterでURLを共有した際に表示されるOGPカードの例](/images/blog/4LAaL3b.png)
...
<meta property="og:image" content="/images/blog/tmkmoVA.png" data-react-helmet="true">
...
<meta name="twitter:image" content="/images/blog/tmkmoVA.png" data-react-helmet="true">
```
...
最後に[GitHub Actions](/2019/09/02/github-action/)に載せて、定期実行させます。
...
![RSS更新をSlackに自動投稿した結果の画面](/images/blog/Ip4IaYs.png)
...
HarperDBを使って何でもつくれそうな予感がするこの頃です。後日、[Denoを使ってGraphQLを叩きながらSlackに投稿する](/2021/07/28/deno-slack/)記事も書いたのでそちらもぜひご覧ください。
