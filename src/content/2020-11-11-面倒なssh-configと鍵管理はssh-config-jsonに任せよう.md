...
Pythonのコード規約チェックはpep8からflake8など多岐にわたり、且つPytestなどのテストランナーと組み合わせることができますが、今回はBlackを使います。
...
まさに、公式も言っている通りフォーマットでのコントロールを完全に譲ることになります。
...
PyPIへのパッケージ登録を行うには当然パッケージを作らないと行けませんが、Pythonの場合setup.pyを記載することで作ることができます。
