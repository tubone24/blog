...
先に結論ですが、こんな感じの企業名を入力することでどんどん企業名の候補がサジェストで出てくるようなUIを作ってほしいとクライアントやプロダクトマネージャーから依頼されたらどうしますか...?
...
- フロントエンドから直接クエリを投げ込んで検索結果にアクセスする
...
のように特に先頭ワイルドカードでQueryを実行すると検索に負荷がかかってしまいます。

めちゃくちゃでかいインスタンスやクラスターを組んで実行していけば解決できそうですが、小さいコンテナで動かすには不向きです。

そもそも、企業サジェストのようにどんどん入力していく形のUIだと、前方から絞り込みがかかるので前方のワイルドカードはそもそも不要だと思います。
...
![](https://i.imgur.com/PQdcbVc.jpg)

今回はこんな感じでECSにすっごいちっちゃいちっちゃいコンテナを立ち上げ、そこであらかじめ企業名をインデックスしておいたElasticsearchを立ち上げる構成を取ります。

また、かなり乱暴ですが、バックエンドAPIとは切り離し直接フロントエンドからElasticsearchのクエリを実行させるようにするため、このあと後述しますが、インデックスを読み取り専用にしております。
