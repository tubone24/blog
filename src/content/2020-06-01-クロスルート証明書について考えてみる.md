...
先程も申した通り、通常ルート証明書はPC側のアップデートに追従して更新されていくのでそこまで気になりませんが、この手の**エージェントやJDK**などは独自にルート証明書を抱えていて、それを使うようにする場合も多いです。
...
この**Tornadoサーバ(Forwarder)**にmetricsのcollectorが収集した情報を投げ込むことでDatadogサーバへmetricsをForwardingしてくれる仕組みのようです。
