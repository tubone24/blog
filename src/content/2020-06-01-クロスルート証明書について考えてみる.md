...
上記で述べた鍵交換一つとってもたとえば**RSA**という方式ではほぼ上記で説明しているとおりの流れを組むのですが、**前方秘匿性**に弱い欠点(一言でいえば全暗号通信を記憶していた状態で秘密鍵を手に入れたらすべてがばれてしまう)があったりで最近ではDHE（DiffieHellman, Ephemeral）という鍵交換方式を楕円曲線暗号上で行なう**ECDHE**（RFC4492）が主流だったりします。
