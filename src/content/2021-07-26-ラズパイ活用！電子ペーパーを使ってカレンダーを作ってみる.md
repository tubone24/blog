...
キースイッチが押されたときに何かをする、というのは**GPIO.add\_event\_detect**でイベントリスナーを登録できます。コールバック関数が設定できるので、こちらにキースイッチが押下された際のふるまいを登録すればいいです。また、コールバック関数にはGPIOのPIN番号が引数でわたりますのでkey_pressedみたいな感じで共通化できます。
