...
WebGUIの他、CIで使いやすいようにCLIも用意されてますが、まずはスキャン内容が見やすいGUIでローカル実行していきます。
...
また、モードをStandard Mode、Active Scan含むにします。
...
Automated ScanのAjax Spiderは、自身のURLサブツリーのみを対象にするらしく、クロールの過程で見つかった外部リンクはOut of Scoreになるようです。そのほうが都合がいいのでありがたいです。

![ajax spider](https://i.imgur.com/i0OIcxd.png)

クロールが終わるとそのままPassive Scan、Active Scanに入ります。
...
　
といった具合に出てきた結果について精査する必要はありますが、ひとまずスキャンはできたようです。

本当は、False positiveになったテストはAlert filterに設定して次回以降のテストに引っかからないようにする必要があります。

GUIで設定したものをCLIに引き継ぐ方法については[こちらのIssue](https://github.com/zaproxy/zaproxy/issues/6909)でやり取りされているものの、まだ自分の方でも上手くいってません。この点は脆弱性診断でも検知率を向上させる重要な機能なので完成次第ブログを更新しようと思います。

### GitHub Actionsに組み込む

さて、GUIでうまくいったScanについてそのままCI(GitHub Actions)にも組み込んでいきます。

とはいってもさきほど起動したDocker containerでPythonコードを実行してあげるだけです。

次のようなActionsを作ることで実現可能です。
...
ポイントはさきほどと異なり、docker composeコマンドで `docker-compose-ci.yml` と別のcomposeファイルを呼び出してますが、こちらは単にttyをoffにしているだけです。
...
という具合で、まず、検査対象のweb-targetコンテナと、owasp zapコンテナを立ち上げます。

その後、execコマンドで **zap-full-scan.py** を実行してあげることでFull Scanが実行されます。

Optionsの説明は[公式のZAP - Full Scan](https://www.zaproxy.org/docs/docker/full-scan/)に記載があります。

`-r` オプションでレポートの出力ができますが、こちら出力したものを後々確認したいので、[peaceiris/actions-gh-pages@v3](https://github.com/peaceiris/actions-gh-pages)を使いGitHub Pagesにアップロードすることにしました。

また、実行はworkflow_dispatchのみにしてます。理由はOWASP ZAPの実行はかなり時間がかかるため、毎度のPRで実行するととてもじゃないですが、耐えられる待ち時間でもないです。

定期的に手動実行という形でいったん進めてますが、余裕のある人は定期実行なんかもで良さそうです。

### 結果の出力

このようにHTML形式で、レポートを確認できます。

![report](https://i.imgur.com/FnGbBym.png)

結構きれいなレポートですね。

[Information Disclosure - Debug Error Messages](https://www.zaproxy.org/docs/alerts/10023/)や [Private IP Disclosure](https://www.zaproxy.org/docs/alerts/2/)はブログ記事やロゴのSVGに含まれる文字列を引っ張って検知してしまった誤検知です。

このあたりをレポートに出さなくする方法はさきほどお話したAlert filterでかいけつできますが、CLIへの渡し方についてはもうちょっと調査が必要です。

レベル高[Cloud Metadata Potentially Exposed](https://www.zaproxy.org/docs/alerts/90034/)が出てしまってますが、こちらはDocker containerの設定によるものでこのブログ自身にはセキュリティイシューのないことを確認済みです。

## 結論

このように、SnykとOWASP ZAPを使えば、それなりにしっかりした脆弱性チェックができることがお分かりいただけたのではないでしょうか。

今年は年始めから色々セキュリティに関するインシデントが乱発してますが皆さんのアプリケーションも「今日も一日ご安全に！」
