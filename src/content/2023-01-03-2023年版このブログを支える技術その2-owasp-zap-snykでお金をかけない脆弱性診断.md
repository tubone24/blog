...
Depandabotも書いてある情報は大差ないのですが、好みの問題でしょうが、Snykのほうが見やすい..。
...
任意のShare Linkが作れてしまうかもしれません。(繰り返しになりますが、不正なURL404 Not Foundページに遷移するので通常ShareBoxは出ませんが...。)
...
力技ですが上のようにSnykコマンドの標準出力をいったんtxtで吐き出して、必要なフォーマット・エスケープ処理を行ったのち、結果をon.pull_requestイベントの際に取れる、github.event.pull_request.comments_urlにPOSTします。
...
そんなツラミを解決してくれる手段の一つが今回ご紹介する[OWASP ZAP](https://www.zaproxy.org/)です。
