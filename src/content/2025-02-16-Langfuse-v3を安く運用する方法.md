...
Langfuse v3になることで、ClickhouseやWorker、Redisなどの追加が必要になるためコストがどうしても上がってしまいます。
...
S3 Table Engineは、S3バケット上のデータをテーブルとして扱うための仕組みです。このエンジンを使用すると、S3のデータを直接クエリできるようになります。

Clickhouseのテーブルの実データをS3に保存することで、ブロックストレージ自体にデータが保存されなくなり、コストを抑えることができます。（メタデータという形でS3への参照情報は保存されます。）
...
<https://github.com/tubone24/clickhouse-server-s3disk/pkgs/container/clickhouse-server-s3disk>

このイメージを使うことで、S3 Diskを使ったClickhouseの運用を簡単に行なうことができます。
...
<https://github.com/orgs/langfuse/discussions/5516>
