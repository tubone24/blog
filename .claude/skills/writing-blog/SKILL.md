...
- コードブロックを使う場合は、適切な言語タグを付与すること（例: ```typescript や```bash など）。
  ...
  - コードブロックの内容が特定のファイルに対応する場合は、言語タグの直後に `{file: "ファイルパス"}` を付与してファイル名ラベルを表示すること。
    - ex: `` ```json{file: "~/.claude/settings.json"} ``
    - ex: `` ```bash{file: "~/.claude/hooks/slack-notify.sh"} ``
    - ex: `` ```typescript{file: "src/index.ts"} ``
    - これにより、コードブロックの右上にファイルパスがラベル表示される（`src/lib/rehype-gatsby-code-meta.mjs` が `data-file` 属性に変換し、CSSでスタイリングされる）。
    - stdinの入力例やコマンド出力など、特定のファイルに対応しないコードブロックにはラベルを付けなくてよい。
...
- 1つの段落（改行なしの連続する文章）が3文以上になる場合は、段落を分割することを検討する
- 特に以下のケースでは積極的に段落を分けること
  - 話題や論点が切り替わるタイミング
  - 「しかし」「一方で」「そして」など接続詞で方向が変わる箇所
  - 強調したい一文を独立した段落にして目立たせる（ex: 「**ならば、通勤時間を少しでも生産的な時間に変えたい**。」）
- 読者がスマートフォンで読むことを想定し、長い段落は避ける
- bad: 「〜です。さらに〜であり、〜もあります。そのため〜ということになります。これにより〜できます。」（1段落に4文以上）
- good: 上記を2〜3段落に分割して、それぞれの主張を明確にする
...
- **重要: 初稿では硬くなりがちなので、意識的にフランクな表現を多めに入れること**
- 具体的なパターン：
  - 三点リーダー（...）を使った余韻
    - ex: 「どうしても焦りを感じてしまいます...。」
    - ex: 「気が焦ってしまうこともありました...。」
  ...
  - 読者への語りかけ・問いかけ
    - ex: 「これ、めちゃくちゃ便利じゃないですか?」
    - ex: 「〜ってどうしてもあるんですよね。」
    - ex: 「〜ですよね。」
  - 話し言葉的な接続
    - ex: 「で、この〜」「さて、ここからが〜」「ちなみに、〜」
    - ex: 「〜なわけです。」「〜というやつです。」
  ...
  - 感嘆表現
    - ex: 「なんと日本の平均通勤時間は往復で**約1時間19分**です!!!」
    - ex: 「これはありがたい。」「楽ちんです。」
- 目安として、1セクションに1〜2個程度。記事全体で5〜10個程度が適切
- 技術的な説明パートでも、ところどころ砕けた表現を挟んで読みやすさを保つこと
...
- 記事全体の文体は「です・ます」調（敬体）で統一すること
- 「〜だ。」「〜である。」「〜い。」（形容詞止め）などの常体（タメ語）は使わない
  - bad: 「これがかなり面白い。」
  - good: 「これがかなり面白いです。」「これがかなり面白いんですよね。」
  - bad: 「地味にこの機能はありがたい。」
  - good: 「地味にこの機能はありがたいです。」
- 砕けた表現は「です・ます」調の範囲内で行うこと（「〜なわけです。」「〜なんですよね。」はOK）
- 見出しは体言止めが基本なので、この制約は本文にのみ適用する
...
- 「」はセリフ（発話・独白・AIへの指示）にのみ使用すること。それ以外の「」は禁止
  - キーワード強調には太字（**）を使う
  - コード出力・UI表示名にはバッククオート（`）を使う
  - bad: 「コーディネーター」と「ワーカー」の2階層で構成しています。
  - good: **コーディネーター**と**ワーカー**の2階層で構成しています。
  - bad: MIDIファイルの「ダウンロード」が可能
  - good: MIDIファイルの**ダウンロード**が可能
  - ok: AIに「最近流行りのJ-POPを作って」と頼んでみたら（セリフなのでOK）
...
- 冒頭文（リード文）で体言止めの指摘表現を使わない
  - bad: 「〜していく。そんな体験ができるXXを作りました。」（体言止め＋「そんな」で繋ぐパターン）
  - good: 季節の話題や個人的な動機から入り、自然に本題に繋げる
  - ex: 「桜も散ってしまい、センチメンタルな曲が聞きたくなりました。」
- 「**Xから**Y**へ**。」のような対比指摘パターンを使わない
  - bad: 「**テキストを返すだけのツール**から**インタラクティブなUIを持つアプリ**へ。」
  - 締めくくりでは、素朴なつぶやきや感想で余韻を残すこと
- 「静かに〜する」「定石」「セオリー」などの格式張った表現は避ける
  - bad: 「静かにスキップする設計です」「定石と言えるでしょう」
  - good: 「スキップしています」「定番です」

### 比喩でごまかさず、そのまま書く

初稿では「気の利いた言い回し」を作りにいってしまいがちだが、筆者の文体ではそれが一番AI臭くなる。
事実をそのまま言うほうが、ほぼ常に良い。

- 見出しで比喩やレトリックを使わない。何の話かをそのまま書く
  - bad: 「## 図で殴ってくる本」
  - good: 「## 図がふんだんに使われているわかりやすい本」
- 動詞を比喩に置き換えない。素直な動詞に戻す
  - bad: 「もう一段深いところまでちゃんとアクセスさせてくれます」
  - good: 「もう一段階深いところまでちゃんと説明してくれます」
  - bad: 「気になったところで立ち止まって降りていける作りです」
  - good: 「気になったところで立ち止まって、深いところまで調べ尽くすことができる作りです」
  - bad: 「この章は効きます」「地味に効きます」
  - good: 「この章はおすすめです」
  - bad: 「逃げずに書いている」「正面から向き合っている」
  - good: 「きちんと書いている」
- 「〜させてくれる」「〜してくれる」の多用に注意する。主語をずらした持ち上げになりやすい
- 英語表現の直訳的な言い回しを使わない
  - ex: 「静かに壊れる」「黙って無視される」「〜した瞬間」

参考にした分析として、[AIの登場前後でQiita記事の書かれ方がどう変わったか](https://nyosegawa.com/posts/qiita-writing-before-after-ai/)がある。
ここで挙げられている変化（太字が約3倍、箇条書きが約1.8倍、地の文が約2倍の長さ、ダッシュが約10倍、1文あたりの読点が0.63→0.95、「効く」が2.0%→22.3%）は、そのままAI臭さの指標になる。
初稿を書いたあとに、太字・箇条書き・読点・「効く」を数えて多すぎないか確認すること。

### 誇張しない

- 量や程度を盛らない。実態に合う言葉を選ぶ
  - bad: 「図がめちゃくちゃ多いです」（多いかどうかは主観で、盛りに見える）
  - good: 「図が的確に使われています」
- 事実として確認できないことを断定しない。断定できないなら書かない

### 自分のブログへの内部リンクを宣伝目的で貼らない

- 過去記事へのリンクは、読者がその記事を読まないと話が通じない場合にだけ貼る
- 「私も書きました」という文脈でのリンクは宣伝になるので貼らない
  - bad: 「私も自分のブログで[AP2に入門する](...)や[MCP Appsの記事](...)を書いていて、そこそこ触ってきたつもりでした」
  - good: 「自分もAP2やMCPに入門してきたつもりですが、」
